      ...
      <main className="mx-auto w-full max-w-6xl px-5 pb-24">
        ...
        {status === "done" && (
          <section className="mt-16 animate-fade-in-up">
            ...
            <div className="mb-8 flex flex-col items-start justify-between gap-4 rounded-3xl border border-primary/20 bg-primary/5 p-5 sm:flex-row sm:items-center">
              <div>
                <p className="text-sm font-semibold text-foreground">Done checking these programs?</p>
                <p className="text-sm text-muted-foreground">
                  Continue with the assistant to confirm eligibility and finish your application step by step.
                </p>
              </div>
              <Link
                to="/chat"
                search={{ q: assistantQuery }}
                className="inline-flex shrink-0 items-center gap-2 rounded-full bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground shadow-sm transition hover:brightness-110"
              >
                <Sparkles className="size-4" /> Continue with the assistant <ArrowRight className="size-4" />
              </Link>
            </div>
              ...
              {cardsWithReasons.map(({ Icon, color, result, ...c }, i) => (
                ...
                >
                  ...
                  <div className="mt-5 flex items-center gap-3">
                    ...
                    <Link
                      to="/chat"
                      search={{ q: `${assistantQuery} — I want to continue applying for ${c.name}.` }}
